...
if __name__ == '__main__':
	...
	with open("labelled_tweets.txt", "w") as output:
		for i in range(len(labelled_tweets)):
			labelled_tweets[i] = labelled_tweets[i].encode('ascii',errors='ignore')
			tweet = labelled_tweets[i].split() 
			cleaned_tweets = [s for s in tweet if not s.startswith("http")]
			for j in range(len(cleaned_tweets)):
				cleaned_tweets[j]=re.sub('[^A-Za-z0-9]+', '', cleaned_tweets[j])
			str1 = ' '.join(cleaned_tweets)
			output.write('{0},{1}\n'.format(str1, 1))
	with open("not_labelled_tweets.txt", "w") as output:
		for i in range(len(all_tweets)):
			all_tweets[i] = all_tweets[i].encode('ascii',errors='ignore')
			tweet2 = all_tweets[i].split() 
			cleaned_tweets2 = [s for s in tweet if not s.startswith("http")]
			for j in range(len(cleaned_tweets2)):
				cleaned_tweets2[j]=re.sub('[^A-Za-z0-9]+', '', cleaned_tweets2[j])
			str12 = ' '.join(cleaned_tweets2)
			output.write('{0},{1}\n'.format(str12, 1))
