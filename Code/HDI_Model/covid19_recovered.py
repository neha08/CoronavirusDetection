...
def fetch_data():
	data = pd.read_csv("time_series_covid_19_recovered.csv")
	j = 4
	n = len(data.columns)
...
def search_location(loc, colname):
	...
	search = 'SELECT * FROM `COVID19_recovered` WHERE `Province`= %s'
